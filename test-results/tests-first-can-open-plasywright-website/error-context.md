...
- Name: tests\first.spec.ts >> can open plasywright website
- Location: tests\first.spec.ts:4:5
...
```ts
  ...
  3  | 
  4  | test('can open plasywright website', async ({ page }) => {
  5  |     const loginPage = new LoginPage(page);
  6  | 
  7  |     await loginPage.navigate();
  8  | 
> 9  |     await expect(page).toHaveTitle(/reliablee/);
     |                        ^ Error: expect(page).toHaveTitle(expected) failed
  10 | });
```